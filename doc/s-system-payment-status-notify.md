...
> **本系统的实现（2026-09 更新）**
>
> 收到 `refunded` / `confused` 时**自动扣减商户余额**并补齐 `order_refunds` / 余额流水，
> 见 `OrderPaymentStatusService::settleGatewayReversal()`。
>
> - 扣款金额取本系统账面的**剩余未退金额**（`amount - refunded_amount`），不取 payload 里的
>   `amount`（该字段按第二节是订单金额而非退款金额）。两者不一致时照常入账，另发 Telegram 告警。
> - 幂等由三层保证：`shouldApply()` 的终态守卫挡重复投递、余额流水的唯一 `idempotency_key`
>   挡重复扣款、`refunded_amount` 累计上限挡超额退款。
> - 自动入账失败（如订单已部分退款又收到 `confused`）时**不抛异常**（否则会返回 5xx 触发无谓的
>   5 次重试），退回"只改状态 + Telegram 告警"，由人工在订单详情页补录。
>
> 更早的版本收到这两个状态时只改订单状态、不动余额，且人工补录入口被状态校验挡死，
> 导致订单卡在"显示已退款、钱还挂在商户余额上"的状态。后台订单列表的「待入账」筛选项
> 用于捞出这批历史数据。
