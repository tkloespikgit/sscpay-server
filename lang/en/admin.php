...
return [
    ...
    'order' => [
        ...
        'filters' => [
            ...
            'settlement_pending' => 'Pending settlement (marked refunded/charged back, not deducted)',
            'customer_email' => 'Customer Email',
        ...
        'actions' => [
            ...
            'upload_logistics_hint' => 'Download the CSV via "Export Logistics Template". Five columns are editable: logistics_company (carrier code, required), tracking_number (required), shipped_at (optional, defaults to the file upload time), tracking_url (optional, left blank keeps the existing link) and remark (optional). All other columns are for reference only and changes to them have no effect. Upload the file unchanged, keeping the header and the note row.',
            'logistics_template_note' => 'Editable columns: logistics_company (carrier code, required), tracking_number (required), shipped_at (optional, format 2026-09-22 15:30:00, defaults to the file upload time), tracking_url (optional, left blank keeps the existing link), remark (optional). All other columns are for reference only and changes have no effect. This row starts with # and is ignored on upload; do not delete the header row.',
    ...
    'order_stats' => [
        'nav_label' => 'Order Stats',
        'title' => 'Order Statistics',
        'empty' => 'No data for the selected period',
        'unknown_dimension' => 'Unknown (channel deleted)',
        'orders_count' => ':count orders',
        'amount_order_hint' => 'Each cell shows amount / order count. Amounts are in USD.',
        'timezone_notice' => 'Dates are bucketed in the system timezone (UTC+8), which may differ by one day from the merchant timezone used for timestamp display.',
        'missing_days' => '⚠️ :count day(s) in this period have no aggregation record (:dates); figures may be understated',
        'periods' => [
            'today' => 'Today',
            'yesterday' => 'Yesterday',
    ...
    'telegram_notification' => [
        ...
        'gateway_reversal_settle_failed' => "⚠️ Gateway refund/chargeback auto-settlement failed\n\nOrder: :order_no\nGateway status: :status\nError: :error\n\nThe order status was updated but the amount has **not** been deducted from the merchant balance. Please settle it manually from the order detail page.",
        'gateway_reversal_amount_mismatch' => "⚠️ Gateway amount differs from the order amount\n\nOrder: :order_no\nGateway amount: :gateway_amount :currency\nOrder amount: :order_amount :currency\n\nSettled using the order amount; please verify both sides.",
    ...
    'finance' => [
        ...
        'refund' => [
            ...
            'settle_notice' => '⚠️ This order was already marked as refunded by the gateway, but the amount has not been deducted from the merchant balance yet. This action records that deduction — it does not refund the customer again.',
            'gateway_auto_reason' => 'Gateway refund (auto-settled)',
        ],
        'chargeback' => [
            'action' => 'Chargeback',
            'reason' => 'Reason',
            ...
            'settle_notice' => '⚠️ This order was already marked as charged back by the gateway, but the amount has not been deducted from the merchant balance yet. This action records that deduction.',
            'gateway_auto_reason' => 'Gateway chargeback (auto-settled)',
        ],
        'withdrawal' => [
